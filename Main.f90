    ...
    real (kind=rk) :: start, finish, moments(2,24), error, test
    ...
    params%db(2) = -8.223557117746168E-002 !-(6.9447e-06)
    ...
    params%delta(1)=-0.00023 !-7.2503e-04 !mean([-0.00023 -0.0005901  -0.00091    -0.00117]);
    ...
    !    end do
    !    test = sum(grids%Agrid(1, :))/size(grids%Agrid(ixt, :))
    action =1
    if (action .EQ. 1) then
        params%nu =  0.466352772226276
        params%beta =   0.999999849491823  !0.999999849491823 !0.98716208644227588 !0.99485273766567861 !0.992!0.988164484822505 just labour!             ! Discount factor
        params%gamma =   1.610884853321348E-003 !1.610884853321348E-003 !2.16705246913580    ! 6.3251268401011834E-003 !3!             ! Coefficient of relative risk aversion
